...
const getConvertibleShips = (ships: MasterDataShip[]) => {
  ...
  const isConvertible = (base: MasterDataShip): boolean => {
    ...
    const setRemodelList = (current: MasterDataShip): MasterDataShip | undefined => {
...
export const createShips = (headerValues: string[], rows: GoogleSpreadsheetRow[], start2: Start2) => {
...
const readShipAttrs = async (
  ...
  const replaceShipType = (str: string) => {
    return ship_types.reduce((current, { name, id }) => current.replace(`"${name}"`, id.toString()), str)
  }

  const replaceShipClass = (str: string) => {
    return ship_classes.reduce((current, { name, id }) => current.replace(`"${name}"`, id.toString()), str)
  }

  const replaceName = (str: string) =>
    ships.reduce((current, ship) => current.replace(`"${ship.name}"`, ship.ship_id.toString()), str)

  const replaceAttr = (str: string) =>
    attrs.reduce((current, attr) => current.replace(RegExp(`\\b${attr.key}\\b`, "g"), attr.expr), str)
...
export const updateShipData = async (doc: GoogleSpreadsheet, start2: Start2) => {
  ...
  const [ships, ship_types, ship_classes] = await Promise.all([
    updateRows(shipsSheet, (rows, sheet) => createShips(sheet.headerValues, rows, start2)),
    updateRows(shipTypesSheet, (rows) => createShipTypes(rows, start2)),
    updateRows(shipClassesSheet, createShipClasses),
  ])

  const ship_attrs = await readShipAttrs(shipAttrsSheet, ships, ship_types, ship_classes)

...
impl ContactAnalysis {
    pub fn new(comp: &Comp) -> Self {
        let side = comp.side();
        let single = analyze_ships_contact_chance(side, comp.planes(false).collect::<Vec<_>>());
...
fn analyze_ships_contact_chance(side: Side, planes: Vec<Plane>) -> Option<Vec<DayContactChance>> {
    if side.is_player() {
        [

...
const PlanAirbaseCard: React.FCX<PlanAirbaseCardProps> = ({
  className,
  airbase,
  updatePlan,
  airbaseKey,
  ...
  return (
    ...
    >
      <AirbaseCard
        label={airbaseKey.replace("a", "基地")}
        airbase={airbase}
        update={updateAirbase}
      />
    </Swappable>
  );
};
  ...
  return (
    <>
      <div css={{ marginRight: 8 }}>
        ...
        <LabeledValue
          label="高高度防空"
          value={plan.highAltitudeInterceptionPower}
        />
      </div>

      <Flexbox className={className}>
        {plan.airbaseEntries.map(([key, airbase]) => (
          <StyledPlanAirbaseCard
            key={key}
            airbase={airbase}
            airbaseKey={key}
            updatePlan={update}

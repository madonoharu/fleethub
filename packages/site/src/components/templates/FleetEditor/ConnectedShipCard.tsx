...
import { FleetState, Ship, ShipKey, ShipState } from "@fleethub/core";
...
const useFleetShipActions = ({
  shipKey,
  updateFleet,
}: Pick<Props, "shipKey" | "updateFleet">) => {
...
const ConnectedShipCard = React.memo<Props>(({ className, ship, ...rest }) => {
  ...
  if (ship) {
    element = (
      <ShipCard ship={ship} update={actions.update} onRemove={actions.remove} />
    );
  } else {
    element = (
      <AddShipButton variant="outlined" fullWidth onClick={Modal.show} />
    );
  }

...
const getAirbaseModeLabel = (mode: Airbase["mode"]) =>
  ({ Sortie: "出撃", Standby: "待機", AirDefense: "防空" }[mode]);
...
const AirbaseCard = React.forwardRef<HTMLDivElement, Props>(
  ({ className, label, airbase, update }, ref) => {
    ...
    return (
      <Paper ref={ref} className={className}>
        ...
        <StyledEquipmentList
          type="airbase"
          equipment={airbase.equipment}
          update={update}
          canEquip={airbase.canEquip}
        />
      </Paper>
    );
  }

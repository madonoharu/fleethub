import { Gear, Ship } from "@fleethub/sim"
import { createEquipmentBonuses, EquipmentBonuses, GearData, ShipData } from "equipment-bonus"
import { isNonNullable, mapValues, GEAR_KEYS, GearKey } from "@fleethub/utils"
...
export const getEbonuses = (ship: Ship) => createEquipmentBonuses(toShipData(ship), getGears(ship))
...
export const makeGetNextEbonuses = (ship: Ship, excludedKey: GearKey) => {
  ...
  return (gear: Gear) => {
    const next = createEquipmentBonuses(shipData, [...filtered, toGearData(gear)])

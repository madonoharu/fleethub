...
import { isNonNullable, ShipType, ShipAttr } from "@fleethub/utils"

type FilterFn = (ship: Ship) => boolean

const getShipGroup = ({ ship_type }: Ship) => {
  if ([ShipType.FBB, ShipType.BB, ShipType.BBV, ShipType.XBB].includes(ship_type)) return "Battleship"
  if ([ShipType.CVL, ShipType.CV, ShipType.CVB].includes(ship_type)) return "AircraftCarrier"
...
export const useShipListState = () => {
  ...
  const masterShips = React.useMemo(() => {
    return master_data.ships.map((ship) => createShip({ ship_id: ship.ship_id })).filter(isNonNullable)

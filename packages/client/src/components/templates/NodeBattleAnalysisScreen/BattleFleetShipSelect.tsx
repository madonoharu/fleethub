...
import { Ship, BattleFleet } from "@fleethub/core"
...
const ShipButton: React.FCX<ShipButtonProps> = ({ className, ship, onClick }) => {
  return (
    <Button className={className} variant="outlined" onClick={onClick}>
      <ShipNameplate shipId={ship.shipId} name={ship.name} banner={ship.banner} />
    </Button>
  )
}
...
const BattleFleetShipSelect: React.FCX<BattleFleetShipSelectProps> = ({ fleet, value, onChange }) => {
  ...
  const mainElement = (
    <Column key="main">
      {main.ships.map((ship, index) => (
        <StyledShipButton key={index} selected={value === ship} ship={ship} onClick={() => handleClick(ship)} />
      ))}
    </Column>
  )
  ...
  return <Root>{side === "Enemy" ? [escortElement, mainElement] : [mainElement, escortElement]}</Root>
}

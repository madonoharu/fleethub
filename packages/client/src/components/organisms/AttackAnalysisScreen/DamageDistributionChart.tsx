...
const DamageDistributionChart: React.FCX<DamageDistributionChartProps> = ({ damage }) => {
  ...
  const keys = damagedStates.keys().sort((a, b) => order.indexOf(a) - order.indexOf(b))
  ...
  return (
    <>
      <ComposedChart layout="vertical" data={data} width={width} height={height}>
        ...
        <Legend formatter={(key: HealthState) => `${key} ${toPercent(rec[key] || 0)}`} />
        {keys.map((key) => (

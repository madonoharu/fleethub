...
const order: AttackResultState[] = ["Miss", "Normal", "Shouha", "Chuuha", "Taiha", "Sunk"]

const AttackResultChart: React.FCX<AttackResultChartProps> = ({ attackResultStates }) => {
  const states = attackResultStates.filter((rate) => rate > 0)

  const keys = states.keys().sort((a, b) => order.indexOf(a) - order.indexOf(b))
  ...
  return (
    <>
      <ComposedChart layout="vertical" data={data} width={width} height={height}>
        <XAxis type="number" hide={true} />
        <YAxis type="category" hide={true} />
        <Legend formatter={(key: HealthState) => `${key} ${toPercent(rec[key] || 0, 3)}`} />
        {keys.map((key) => (
          <Bar key={key} dataKey={key} stackId="a" barSize={10} fill={getColor(key)} />
        ))}
      </ComposedChart>
    </>
  )

...
import { RateMap, AntiAirCutin } from "@fleethub/core"
...
const AntiAirCutinChanceChart: React.FCX<Props> = ({ className, label, rateMap }) => {
  const width = 360
  const height = 240
  const cx = width / 2
  const cy = height / 2
  ...
  return (
    <PieChart className={className} width={width} height={height}>
      ...
      >
        ...
        {label && <Label value={label} dy={-10} fontSize="0.75rem" fill="white" position="center" />}
        <Label value={`合計 ${toPercent(rateMap.total)}`} dy={10} fill="white" position="center" />
      </Pie>
    </PieChart>
  )
}
